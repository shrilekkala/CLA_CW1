...
B[0, p+1:] = - B[0, :p+1]
B[1, :p+1] = np.multiply(np.array([1 * data[49,0]**(i-1) for i in range(p+1)]), np.arange(p+1))
...
y_vals[100:] = Vand(x_vals[100:], p+1) @ x[p+1:]
plt.plot(x_vals[:101], y_vals[:101])
plt.plot(x_vals[99:], y_vals[99:])
"""
